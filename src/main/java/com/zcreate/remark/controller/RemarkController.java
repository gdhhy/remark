...
public class RemarkController {
    ...
    public void getRecipeExcel(HttpServletResponse response, @RequestParam(value = "recipeID") int recipeID) throws IOException {
        ...
        int startRow = 3, cellIndex = 3;
//基本情况
        StringBuilder line = Optional.ofNullable(sheet.getRow(startRow).getCell(cellIndex).getStringCellValue()).map(StringBuilder::new)
                .orElse(new StringBuilder("性别 %s  年龄 %s  体重 %s kg   入院时间： %s 出院时间： %s"));
        JsonObject baseInfo = json.getAsJsonObject("基本信息");
        sheet.getRow(startRow++).getCell(cellIndex).setCellValue(String.format(line.toString(), baseInfo.get("sex").getAsString(), baseInfo.get("age").getAsString(),
...
//过敏史
        String string = Optional.ofNullable(sheet.getRow(startRow).getCell(cellIndex).getStringCellValue()).orElse("无{0}　有{1}（抗菌药品通用名：{2}）");
...
//用药情况
        ...
        for (int i = 0; i < drugs.size() * 2; i += 2) {
            ...
            /*aRow = sheet.getRow(startRow + i);
            for (int m = 3; m < sampleRow.getLastCellNum(); m++) {
                aRow.getCell(m).setCellValue(m);
                aRow2.getCell(m).setCellValue(m + "-");
            }
            log.debug("firstCellNum3:" + aRow.getFirstCellNum());
            log.debug("lastCellNum3:" + aRow.getLastCellNum());*/
